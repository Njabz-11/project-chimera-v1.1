...
class OrchestratorAgent(BaseAgent):
    ...
    async def submit_job(self, agent_name: str, job_data: Dict[str, Any]) -> str:
        """Submit a job to a specific agent via Celery queue"""
        import uuid
        ...
        # Submit job to Celery queue (proper decoupled architecture)
        try:
            from workers.agent_tasks import execute_agent_job

            # Submit to Celery queue with appropriate priority
            queue_name = "high_priority" if job_data.get("priority", 5) <= 2 else "agent_jobs"

            execute_agent_job.apply_async(
                args=[agent_name, job_data],
                queue=queue_name,
                priority=job_data.get("priority", 5)
            )

            self.logger.info(f"📤 Job {job_id} queued for {agent_name} in {queue_name}")

        except Exception as e:
            self.logger.error(f"Failed to queue job {job_id}: {e}")
            # Fallback to direct execution if queue is unavailable
            asyncio.create_task(self._execute_agent_job_fallback(agent_name, job_data))

        return job_id

    async def _execute_agent_job_fallback(self, agent_name: str, job_data: Dict[str, Any]):
        """Fallback method for direct job execution when queue is unavailable"""
        try:
            # Create job object
            job = AgentJob(
                job_id=job_data.get("job_id", "fallback"),
            ...
            if agent_name == "SCOUT":
                ...
                agent = GuardianAgent(self.db_manager)
            elif agent_name == "ARCHITECT":
                from .strategist import StrategistAgent
                agent = StrategistAgent(self.db_manager)
            else:
            ...
            await self.db_manager.log_agent_activity(
                ...
                activity_type="job_execution_fallback",
                description=f"Executed job {job.job_id} (fallback mode)",
            ...
            self.logger.error(f"Failed to execute fallback job for {agent_name}: {e}")

            # Log the error
            await self.db_manager.log_agent_activity(
                agent_name=agent_name,
                activity_type="job_execution_fallback",
                description=f"Failed to execute fallback job",
                status="error",
                input_data=job_data,
                error_message=str(e)
            )
