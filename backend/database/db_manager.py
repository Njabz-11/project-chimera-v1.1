...
class DatabaseManager:
    """Manages database operations for Project Chimera (PostgreSQL preferred, SQLite fallback)"""
    ...
    def __init__(self, db_path: str = "data/chimera_local.db"):
        ...
        # Log deprecation warning for SQLite usage
        self.logger.warning("SQLite database manager is deprecated. Please migrate to PostgreSQL for Genesis compliance.")
    ...
    async def _create_default_tenant(self):
        """Create default tenant if not exists"""
        try:
            cursor = await self.connection.execute("SELECT COUNT(*) FROM tenants")
            count = await cursor.fetchone()
            if count[0] == 0:
                await self.connection.execute("""
                    INSERT INTO tenants (id, name, domain, status, plan, max_users, max_missions, max_leads, settings, created_at)
                    VALUES (1, 'Default Tenant', 'localhost', 'active', 'enterprise', 100, 100, 10000, '{}', CURRENT_TIMESTAMP)
                """)
                await self.connection.commit()
                self.logger.info("✅ Default tenant created")
        except Exception as e:
            self.logger.warning(f"Could not create default tenant: {e}")
    ...
    async def _create_tables(self):
        ...
        # Update users table to include tenant_id (if not exists)
        try:
            await self.connection.execute("""
        ...
        # Add missing columns to existing mission_briefings table (migration)
        try:
            await self.connection.execute("ALTER TABLE mission_briefings ADD COLUMN created_by INTEGER")
        except:
            # Column already exists, ignore
            pass

        try:
            await self.connection.execute("ALTER TABLE mission_briefings ADD COLUMN created_by_username TEXT")
        except:
            # Column already exists, ignore
            pass
    ...
    async def create_mission(self, briefing_data: Dict[str, Any]) -> int:
        ...
        cursor = await self.connection.execute("""
            ...
            (business_goal, target_audience, brand_voice, service_offerings, contact_info, created_by, created_by_username)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (
            ...
            briefing_data.get("created_by"),
            briefing_data.get("created_by_username")
        ))
        await self.connection.commit()
    ...
    async def update_mission_status(self, mission_id: int, status: str):
        ...
        await self.connection.execute("""
            UPDATE mission_briefings SET status = ?, updated_at = CURRENT_TIMESTAMP
            WHERE id = ?
        """, (status, mission_id))
        await self.connection.commit()
