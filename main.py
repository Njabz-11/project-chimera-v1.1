...
async def root():
    ...
    <html>
        <head>
            <title>Project Chimera ABOP</title>
        </head>
        <body>
            <h1>🤖 Project Chimera - Autonomous Business Operations Platform</h1>
            <p>The digital workforce is online and ready for autonomous operations.</p>
            ...
            <ul>
                <li><code>ws://localhost:8000/ws/logs</code> - Real-time log streaming</li>
            </ul>
        </body>
    </html>
    """
...
async def create_mission(
    ...
    _rate_limit: bool = Depends(rate_limit(max_requests=10, window_seconds=3600))  # 10 missions per hour
):
...
async def get_leads(
    ...
    if current_user.get("role") == "admin":
        leads = await db_manager.get_all_leads(mission_id)
    else:
